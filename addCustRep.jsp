...
<body>
	...
	try {
		...
		str = "SELECT username FROM customerRep ORDER BY username";
		//Run the query against the DB
		...
		str = "SELECT email FROM customerRep ORDER BY email";
		result = stmt.executeQuery(str);
		count = 0;
		...
		if(searchIndex != -1 || emailIndex != -1){
			response.sendRedirect("addCustRepForm.jsp");
		}
		else{
		//Make an insert statement for the Sells table:
		String insert = "INSERT INTO customerRep(username, password, email)"
				+ "VALUES (?, ?, ?)";
	...
	response.sendRedirect("custRepAccounts.jsp");
	%>
</body>
</html>